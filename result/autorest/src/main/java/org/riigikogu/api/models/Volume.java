/**
 * Code generated by Microsoft (R) AutoRest Code Generator.
 * Changes may cause incorrect behavior and will be lost if the code is
 * regenerated.
 */

package org.riigikogu.api.models;

import java.util.List;
import org.joda.time.DateTime;
import com.fasterxml.jackson.annotation.JsonProperty;

/**
 * Toimik.
 */
public class Volume {
    /**
     * The _links property.
     */
    @JsonProperty(value = "_links")
    private List<Link> _links;

    /**
     * UUID.
     */
    @JsonProperty(value = "uuid")
    private String uuid;

    /**
     * Viit.
     */
    @JsonProperty(value = "reference")
    private String reference;

    /**
     * Pealkiri.
     */
    @JsonProperty(value = "title")
    private String title;

    /**
     * Tüüp.
     */
    @JsonProperty(value = "volumeType")
    private String volumeType;

    /**
     * Loomise aeg.
     */
    @JsonProperty(value = "created")
    private DateTime created;

    /**
     * Registreerimise number.
     */
    @JsonProperty(value = "mark")
    private Integer mark;

    /**
     * Koosseisu number.
     */
    @JsonProperty(value = "membership")
    private Integer membership;

    /**
     * Aasta.
     */
    @JsonProperty(value = "year")
    private Integer year;

    /**
     * Seotud dokumendid.
     */
    @JsonProperty(value = "relatedDocuments")
    private List<BaseDocument> relatedDocuments;

    /**
     * Seotud toimikud.
     */
    @JsonProperty(value = "relatedVolumes")
    private List<BaseVolume> relatedVolumes;

    /**
     * Dokumendid.
     */
    @JsonProperty(value = "documents")
    private List<Document> documents;

    /**
     * Get the _links value.
     *
     * @return the _links value
     */
    public List<Link> _links() {
        return this._links;
    }

    /**
     * Set the _links value.
     *
     * @param _links the _links value to set
     * @return the Volume object itself.
     */
    public Volume with_links(List<Link> _links) {
        this._links = _links;
        return this;
    }

    /**
     * Get uUID.
     *
     * @return the uuid value
     */
    public String uuid() {
        return this.uuid;
    }

    /**
     * Set uUID.
     *
     * @param uuid the uuid value to set
     * @return the Volume object itself.
     */
    public Volume withUuid(String uuid) {
        this.uuid = uuid;
        return this;
    }

    /**
     * Get viit.
     *
     * @return the reference value
     */
    public String reference() {
        return this.reference;
    }

    /**
     * Set viit.
     *
     * @param reference the reference value to set
     * @return the Volume object itself.
     */
    public Volume withReference(String reference) {
        this.reference = reference;
        return this;
    }

    /**
     * Get pealkiri.
     *
     * @return the title value
     */
    public String title() {
        return this.title;
    }

    /**
     * Set pealkiri.
     *
     * @param title the title value to set
     * @return the Volume object itself.
     */
    public Volume withTitle(String title) {
        this.title = title;
        return this;
    }

    /**
     * Get tüüp.
     *
     * @return the volumeType value
     */
    public String volumeType() {
        return this.volumeType;
    }

    /**
     * Set tüüp.
     *
     * @param volumeType the volumeType value to set
     * @return the Volume object itself.
     */
    public Volume withVolumeType(String volumeType) {
        this.volumeType = volumeType;
        return this;
    }

    /**
     * Get loomise aeg.
     *
     * @return the created value
     */
    public DateTime created() {
        return this.created;
    }

    /**
     * Set loomise aeg.
     *
     * @param created the created value to set
     * @return the Volume object itself.
     */
    public Volume withCreated(DateTime created) {
        this.created = created;
        return this;
    }

    /**
     * Get registreerimise number.
     *
     * @return the mark value
     */
    public Integer mark() {
        return this.mark;
    }

    /**
     * Set registreerimise number.
     *
     * @param mark the mark value to set
     * @return the Volume object itself.
     */
    public Volume withMark(Integer mark) {
        this.mark = mark;
        return this;
    }

    /**
     * Get koosseisu number.
     *
     * @return the membership value
     */
    public Integer membership() {
        return this.membership;
    }

    /**
     * Set koosseisu number.
     *
     * @param membership the membership value to set
     * @return the Volume object itself.
     */
    public Volume withMembership(Integer membership) {
        this.membership = membership;
        return this;
    }

    /**
     * Get aasta.
     *
     * @return the year value
     */
    public Integer year() {
        return this.year;
    }

    /**
     * Set aasta.
     *
     * @param year the year value to set
     * @return the Volume object itself.
     */
    public Volume withYear(Integer year) {
        this.year = year;
        return this;
    }

    /**
     * Get seotud dokumendid.
     *
     * @return the relatedDocuments value
     */
    public List<BaseDocument> relatedDocuments() {
        return this.relatedDocuments;
    }

    /**
     * Set seotud dokumendid.
     *
     * @param relatedDocuments the relatedDocuments value to set
     * @return the Volume object itself.
     */
    public Volume withRelatedDocuments(List<BaseDocument> relatedDocuments) {
        this.relatedDocuments = relatedDocuments;
        return this;
    }

    /**
     * Get seotud toimikud.
     *
     * @return the relatedVolumes value
     */
    public List<BaseVolume> relatedVolumes() {
        return this.relatedVolumes;
    }

    /**
     * Set seotud toimikud.
     *
     * @param relatedVolumes the relatedVolumes value to set
     * @return the Volume object itself.
     */
    public Volume withRelatedVolumes(List<BaseVolume> relatedVolumes) {
        this.relatedVolumes = relatedVolumes;
        return this;
    }

    /**
     * Get dokumendid.
     *
     * @return the documents value
     */
    public List<Document> documents() {
        return this.documents;
    }

    /**
     * Set dokumendid.
     *
     * @param documents the documents value to set
     * @return the Volume object itself.
     */
    public Volume withDocuments(List<Document> documents) {
        this.documents = documents;
        return this;
    }

}
